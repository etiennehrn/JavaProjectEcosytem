...
import static com.etienne.ecosysteme.environment.HerbeType.VariantHerbe;
...
public class MapGeneration extends PApplet {
    ...
    public void displayMap(int titleSize, Player player) {
        ...
        // Limites Affichages
        int startCol = Math.max(player.getCol() - player.getVisionRange(), 0);
        int startRow = Math.max(player.getRow() - player.getVisionRange(), 0);
        int endCol = Math.min(player.getCol() + player.getVisionRange(), cols);
        int endRow = Math.min(player.getRow() + player.getVisionRange(), rows);
